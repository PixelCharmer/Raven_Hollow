...
export default function Room5Lock({
    ...
    // basic keyboard support (optional nicety)
    useEffect(() => {
        let focusIndex = 0;
        const onKey = (e) => {
                ...
                <div className="digits">
                    {digits.map((d, i) => (
                        <div className="digit" key={i}>
                            <button className="arrow up" onClick={() => bump(i, +1)} aria-label={`Increase digit ${i + 1}`}>▲</button>
                            <div className="value-wrap" aria-live="polite">
                                <div className="value-shadow" />
                                <div className="value">{d}</div>
                            </div>
                            <button className="arrow down" onClick={() => bump(i, -1)} aria-label={`Decrease digit ${i + 1}`}>▼</button>
                        </div>
                    ))}
                </div>

                <div className="pad-actions">
                    <button className="btn back" onClick={() => nav(backRoute)}>Back</button>
